...
from utils.mask.read_cfg import read_cfg
from utils.mask.fit_ellipse import *
import random
from utils.mask.create_mask import texture_the_mask, color_the_mask
...
def get_six_points(face_landmark, image, type):
...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    if not "empty" in type and not "inpaint" in type:
        cfg = read_cfg(config_filename="utils/mask/masks.cfg", mask_type=type, verbose=False)
    else:
        if "left" in type:
    ...
    # draw_six_points_on_face(mask_line, img)
    # Warp the mask
...
def draw_landmarks(face_landmarks, image):
    ...
    # pil_image.show()
    pil_image.save(f'./draw_landmarks{str(int(face_landmarks[facial_feature][0]))}.png')


def draw_six_points_on_face(six_points_on_face, image):
    rgb_image = image.copy()
    rgb_image[:,:,[0,1,2]] = image[:,:,[2,1,0]]
    pil_image = Image.fromarray(rgb_image)
    d = ImageDraw.Draw(pil_image)
    for i, points in enumerate(six_points_on_face):
        d.point(points, fill="white")
        d.text(points, str(i), fill="red")
    # pil_image.show()
    pil_image.save(f'./draw_six_points_on_face_{str(int(points[0]))}.png')
...
def mask_image(image, args):
    ...
    face_location = dlib.rectangle(0, 0, image.shape[1], image.shape[0])
    shape = args.predictor(gray, face_location)
    shape = face_utils.shape_to_np(shape)
    face_landmarks = shape_to_landmarks(shape)
    # draw_landmarks(face_landmarks, image)
    six_points_on_face, angle = get_six_points(face_landmarks, image, type=mask_type)
    # draw_six_points_on_face(six_points_on_face, image)
    mask = []
    if mask_type != "all":
...
def get_available_mask_types(config_filename="utils/masks/masks.cfg"):
    parser = ConfigParser()
